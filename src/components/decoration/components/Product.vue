<template>
  <div class="wrap">
    <div
      :class="[options.type!=1?'type'+options.type:'',options.border!='0'?'border'+options.border:'']"
    >
      <div v-for="(item,index) in product" :key="index">
        ...
        <div>
          ...
          <div>
            ￥{{item.price.money}}
            <span>+{{item.price.points}}积分</span>
          </div>
          <div>
